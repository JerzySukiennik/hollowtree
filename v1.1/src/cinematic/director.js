...
// The authored position of a shot at absolute time t, written into `out`. Both the live camera and
// the offline path check (intro.samplePath) go through this, so a clearance test can never drift
// away from what actually renders. Returns null when the shot authors no position.
export function shotPosition(shot, t, context, out) {
  ...
  if (typeof shot.posFn === 'function') return out.copy(shot.posFn(u, context, t, local));
  if (shot.posTrack) {
    shot.posTrack.at(out, u);
    return out;
  }
  return null;
}
...
export function createDirector(options) {
  ...
  function applyCamera(shot, t) {
    ...
    if (!shotPosition(shot, t, context, _pos)) return;
