...
export function createMenu(options) {
  ...
  // The queen is built at boot, long before this screen exists, so the livery travels to it as an
  // event rather than an argument. It repaints live here and survives a reload through the same
  // localStorage record persist() just wrote.
  function announceProfile() {
    if (typeof window === 'undefined' || typeof window.CustomEvent !== 'function') return;
    window.dispatchEvent(new CustomEvent('hollowtree:profile', { detail: { ...profile } }));
  }
  ...
  function showPanel(next) {
    // The old code only stripped is-on when moving to a *different* panel, so closing the open one
    // left it on screen — which is exactly what made Back and Escape look dead. Close is now
    // unconditional, and every path (Back, Escape, re-clicking the nav item) runs through it.
  ...
  function closePanel() {
    ...
    openPanel = null;
    for (const item of items) item.button.classList.remove('is-active');
    // Back returns you to the screen you came from — the nav — with the cursor on the way in.
    if (openedFrom && document.body.contains(openedFrom)) openedFrom.focus({ preventScroll: true });
    openedFrom = null;
  }

  function setPanel(next) {
    if (!next || openPanel === next) closePanel();
    else showPanel(next);
  }
  ...
  function navItem(label, note, panel, action) {
    ...
    button.addEventListener('click', () => {
      if (panel) {
        if (openPanel !== panel) openedFrom = button;
        setPanel(panel);
  ...
  function show() {
    ...
    visible = true;
    // The fade-in is deferred a frame so the transition has something to run from. If hide() lands
    // in that gap — or if the tab was throttled and the callback is stale — the menu must not
    // paint itself back over the world, which is what put it on top of the opening cinematic.
