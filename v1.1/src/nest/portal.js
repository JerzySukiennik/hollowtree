...
export function createPortal(scene, interior, flight, detail) {
  ...
  // The hall is deliberately larger than the trunk, so it is only ever safe to draw while
  // the VIEWER is inside it. In play the camera trails the queen and the two agree, which
  // is why sampling the queen worked — until the cinematic, where the director keeps the
  // queen in the hall and flies the camera outside for the establishing shots. The hall
  // then stayed visible and punched through the bark as a slab of comb beside the entrance.
  // A cinematic sets the viewpoint to its camera; everything else keeps the queen.
  let viewpoint = null;
  ...
  // Plain containment in the hall volume, with no path memory. sample() above is a latch:
  // it only ever reads 1 for a body that was seen crossing the aperture, which is right for
  // a queen who must fly in and wrong for a cinematic camera that simply begins inside.
  function insideHall(p) {
    return p.x > spec.minX && p.x < spec.maxX
      && p.z > spec.minZ && p.z < spec.maxZ
      && p.y > spec.floorY && p.y < spec.ceilY;
  }
  ...
  function update(dt) {
    const override = viewpoint ? viewpoint() : null;
    const point = override || (flight ? flight.simPosition || flight.position : null);
    if (!point) return state;
    // The latch is a path memory, so it has to be dropped when the sampled point jumps
    // between two different bodies — otherwise the queen's latch survives the swap and
    // the hall reads as "inside" for a camera standing out on the meadow.
    if (override !== lastOverride) {
      lastOverride = override;
      // Re-derive the latch for whichever body is being sampled now. Clearing it outright
      // would drop the hall out from under a queen who is already standing in it when the
      // cinematic hands control back.
      state.latched = insideHall(point);
    }
    state.target = override ? (insideHall(point) ? 1 : 0) : sample(point);
  ...
  return {
    state,
    update,
    // Pass a function returning the point the viewer occupies, or null to go back to the
    // queen. Used by the opening cinematic, whose camera leaves the queen behind.
    setViewpoint(fn) {
      viewpoint = typeof fn === 'function' ? fn : null;
    },
    get insideness() {
