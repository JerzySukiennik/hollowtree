...
export function createComb(options) {
  ...
  function seedPlan() {
    ...
    // Walk outward from the aperture centre along the wall. isValid() already excludes the
    // aperture box, so the first cell it accepts IS the rim — no need to model the hole here.
    let anchor = null;
    for (let step = 0; step < steps && !anchor; step++) {
      const s = entrance.s + side * (clear + (step + 0.5) * grid.cellWidth);
      const c = grid.seed(s, entrance.y, 0);
