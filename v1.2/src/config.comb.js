...
export const COMB_TUNING = {
  ...
  //
  // It hugs the aperture rim rather than sitting on the shelf, because that is the one
  // patch of wall the arriving queen is guaranteed to fly past, it is inside the throw of
  // the entrance light, and it puts routeDist 0 on the comb so the route-gated defensive
  // cells have somewhere legal to go from the first minute.
  seed: {
    type: 'seed',
    owner: 'hive',
    cells: 7,          // anchor + its six neighbours: one hex flower
    side: -1,          // which way along the wall from the aperture centre (-1 or +1)
    probeSteps: 14,    // how far out to walk looking for the first buildable rim cell
  },
