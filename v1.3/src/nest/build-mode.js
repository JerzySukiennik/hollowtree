...
export function createBuildMode(options) {
  ...
  // ------------------------------------------------------------- legal-spot hints
  //
  // Measured on a fresh hive: of 168 aim directions from where the queen arrives, 151 land
  // inside the entrance keep-clear zone, 14 touch nothing, 3 are legal. So the player sweeps
  // the wall, sees red almost everywhere, and has no way to find the three. These markers
  // answer "where", which no message can. They are rebuilt on open and on comb changes, never
  // per frame, and they show spots that fail only on cost as well — "here, once you have
  // resin" is exactly the thing worth knowing while the stores are empty.

  const HINT_MAX = 192;
  let hints = null;
  if (scene) {
    const hintGeo = hexCellGeometry(grid.cellSize() * 0.82, COMB.cellDepth * 0.22);
    hints = new THREE.InstancedMesh(
      hintGeo,
      new THREE.MeshBasicMaterial({
        color: COMB_TUNING.colors.ghostValid,
        transparent: true,
        // The hollow interior is lit warm gold; a pale-green marker at 0.16 was invisible
        // against it, which defeats the whole point of drawing one.
        opacity: 0.55,
        depthWrite: false,
  ...
  const _hintQuat = new THREE.Quaternion();
  const _hintScale = new THREE.Vector3(1, 1, 1);
  const _hintMat = new THREE.Matrix4();
  ...
  function rebuildHints() {
    ...
    for (const cell of comb.cells.values()) {
      const list = grid.neighbors(cell.col, cell.row, cell.layer);
      for (let i = 0; i < list.length && n < HINT_MAX; i++) {
        const c = list[i];
        const k = grid.key(c.col, c.row, c.layer);
        if (seen.has(k)) continue;
        seen.add(k);
        const check = comb.canPlace(c.col, c.row, c.layer, state.typeId);
        // `cost` means the spot itself is legal and only the stores are short.
        if (!check.ok && check.reason !== 'cost') continue;
        grid.cellCenter(c.col, c.row, c.layer, _hintPos);
        grid.cellBasis(c.col, c.row, _hintQuat);
        _hintMat.compose(_hintPos, _hintQuat, _hintScale);
        hints.setMatrixAt(n, _hintMat);
        n++;
      }
      if (n >= HINT_MAX) break;
    }
    ...
    hints.visible = state.open && n > 0;
  }

  // Placing or tearing down a cell changes where the next one may go. Guarded on `open`
  // because comb fires this on ripening ticks too, and rebuilding then would be pure waste.
  const stopCombWatch = typeof comb.onChange === 'function'
    ? comb.onChange(() => { if (state.open) rebuildHints(); })
    : null;
  ...
  function refresh() {
    ...
    if (!state.target) {
      state.valid = false;
      state.reason = 'aim';
      state.reasonText = 'look at the wall of the hollow';
    ...
    // The panel used to print the raw enum, so aiming anywhere near the doorway just said
    // "blocked" — a word that names the rule without naming the reason. The readable copy
    // already existed in DENIED; it was only being used for a failed click.
    state.reasonText = check.reason ? DENIED[check.reason] || check.reason : '';
  ...
  return {
    ...
    dispose() {
      ...
      if (stopCombWatch) stopCombWatch();
      if (hints) {
        if (hints.parent) hints.parent.remove(hints);
        hints.geometry.dispose();
        hints.material.dispose();
        hints.dispose();
      }
    },
  };
